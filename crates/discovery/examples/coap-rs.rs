    ...
    Runtime::new().unwrap().block_on(async move {
        ...
        server.run(|request| async {
            match request.get_method() {
                ...
                &RequestType::Post => println!("request by post {}", String::from_utf8(request.message.payload).unwrap()),
                &RequestType::Put => println!("request by put {}", String::from_utf8(request.message.payload).unwrap()),
